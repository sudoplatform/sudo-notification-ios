...
/// Transformer for GraphQL types to output results of the SDK.
    func transform(_ result: GraphQL.NotificationSettingsOutput) -> NotificationConfiguration {
        ...
        for conf in configData {
            ...
            if let filterActionRawValue = conf.snapshot["actionType"] as? String,
               let filterAction = GraphQL.FilterAction(rawValue: filterActionRawValue)
            {
